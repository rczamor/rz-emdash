...
async function resolveSkills(
	...
	for (const slug of agent.skills) {
		try {
			...
			const name = String(data.title ?? data.name ?? slug);
...
async function putMemory(input: MemoryPutInput, ctx: PluginContext): Promise<MemoryEntry> {
	...
	const existing = await ctx.storage.memory.query({
		filter: { agent_id: input.agent_id, key: input.key },
		limit: 1,
	});
...
async function searchMemory(input: MemorySearchInput, ctx: PluginContext): Promise<MemoryEntry[]> {
	...
	const filter: Record<string, unknown> = { agent_id: input.agent_id };
	...
	routes: {
		...
		"memory.list": {
			handler: async (routeCtx: RouteCtx, ctx: PluginContext) => {
				...
				return { entries: result.items.map((i) => i.data), cursor: result.cursor, hasMore: result.hasMore };
			},
		},
		...
		"memory.delete": {
			handler: async (routeCtx: RouteCtx, ctx: PluginContext) => {
				...
				if (body.agent_id && body.key) {
					const result = await ctx.storage.memory.query({
						filter: { agent_id: body.agent_id, key: body.key },
						limit: 1,
					});
					const entry = result.items[0]?.data as MemoryEntry | undefined;
						...
						await createAgent(
							{
								...
								identity: String(v.identity ?? "").trim(),
								soul: v.soul ? String(v.soul) : undefined,
								model: { primary: String(v.model_primary ?? "anthropic/claude-haiku-4-5") },
						...
						await updateAgent(
							{
								...
								identity: v.identity !== undefined ? String(v.identity) : undefined,
								soul: v.soul !== undefined ? String(v.soul) : undefined,
								model: v.model_primary ? { primary: String(v.model_primary) } : undefined,
